...
class WriterAI(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `api_key` argument from the `WRITERAI_AUTH_TOKEN` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("WRITERAI_AUTH_TOKEN")
        if api_key is None:
            raise WriterAIError(
                "The api_key client option must be set either by passing api_key to the client or by setting the WRITERAI_AUTH_TOKEN environment variable"
            )
