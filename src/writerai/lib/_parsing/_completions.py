...
def parse_chat_completion(
    *,
    ...
    input_tools: Iterable[ChatCompletionToolParam] | Omit,
...
def has_parseable_input(
    *,
    ...
    input_tools: Iterable[ChatCompletionToolParam] | Omit = omit,
) -> bool:
    if has_rich_response_format(response_format):
        return True
...
def _parse_content(response_format: type[ResponseFormatT], content: str) -> ResponseFormatT:
    ...
    if is_dataclass_like_type(response_format):
        if PYDANTIC_V1:
            raise TypeError(f"Non BaseModel types not supported with Pydantic v1 - {response_format}")
